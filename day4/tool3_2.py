...
def qa_pipeline_09_qa_extractive(*, question, context, tokenizer, qa_model, max_answer_len=30):
    encoded = tokenizer(
        question,
        context,
        return_tensors='pt',
...
def load_translation_pipeline(model_name, src_lang=None, tgt_lang=None):
    """Load a Seq2Seq translation model as a pipeline-compatible callable.

    transformers 5.x removed the "translation" pipeline task, so this wraps
    AutoModelForSeq2SeqLM + AutoTokenizer with a `pipeline("translation", ...)`-style
    interface: call it with a string or list of strings and get back a list of
    {"translation_text": ...} dicts.

    For multilingual models (e.g. `facebook/nllb-200-distilled-600M`), pass
    `src_lang`/`tgt_lang` language codes (e.g. "eng_Latn"/"kor_Hang") so the
    tokenizer and `forced_bos_token_id` are set correctly.
    """
    import torch
    from transformers import AutoModelForSeq2SeqLM, AutoTokenizer

    tokenizer_kwargs = {"src_lang": src_lang} if src_lang else {}
    ...
    def translate(texts, max_length=512):
        inputs_list = [texts] if isinstance(texts, str) else list(texts)
...
korean_news = """
국내 주요 IT 기업들이 인공지능(AI) 기술 개발에 대규모 투자를 단행하고 있다.
삼성전자는 올해 AI 반도체 연구개발(R&D) 예산을 전년 대비 40% 늘린 5조 원으로 책정했다고
밝혔다. 회사 측은 온디바이스 AI와 데이터센터용 고성능 칩 개발에 집중할 계획이라고 설명했다.
SK텔레콤은 자체 개발한 초거대 언어 모델 '에이닷(A.)'의 성능을 대폭 업그레이드하고,
연내 B2B 서비스를 공식 출시할 예정이다. 통신·의료·금융 분야 맞춤형 AI 솔루션을 제공해
기업 고객을 대상으로 한 구독 매출을 확대한다는 전략이다.
네이버는 하이퍼클로바X를 기반으로 한 AI 검색 서비스를 전면 개편하면서 이용자 체류 시간이
25% 증가했다고 발표했다. 네이버 관계자는 "AI가 검색 결과를 단순 나열하는 것을 넘어
사용자 의도를 파악해 최적의 답변을 생성하는 방식으로 진화하고 있다"고 말했다.
카카오는 최근 AI 자회사 카카오브레인을 본사에 흡수합병하고, AI 사업 조직을 하나로 통합했다.
이를 통해 개발 속도를 높이고 중복 투자를 줄이겠다는 방침이다. 카카오는 멀티모달 AI 모델
개발에 역량을 집중해 2026년까지 글로벌 시장 진출을 목표로 하고 있다.
전문가들은 국내 AI 경쟁이 점차 격화되면서 핵심 인재 확보가 기업 성패를 좌우할 것이라고
전망했다. 한 AI 연구소장은 "데이터와 컴퓨팅 자원만큼 우수한 연구 인력이 중요하다.
지금이 AI 인재에 투자할 최적의 시기"라고 강조했다.
정부도 이에 발맞춰 AI 기술 개발 지원 예산을 대폭 늘리고, 규제 샌드박스를 통해
혁신적인 AI 서비스가 빠르게 시장에 나올 수 있도록 지원할 방침이다.
""".strip()


english_text = (
    "Artificial intelligence is transforming industries at an unprecedented pace. "
    "Companies across the globe are investing billions of dollars in AI research and development. "
    "From healthcare to finance, AI models are automating complex tasks, improving efficiency, "
    "and generating insights that were previously impossible to obtain. "
    "However, experts warn that the rapid adoption of AI also brings challenges, "
    "including concerns about job displacement, data privacy, and algorithmic bias. "
    "Governments are beginning to draft regulations to ensure that AI development "
    "remains ethical and benefits society as a whole."
)


@lru_cache(maxsize=1)
def _load_ko_summarizer():
    import torch
...
def summarizer_ko(
    ...
    **generate_kwargs,
    ...
    """Return output in pipeline-like format: [{"summary_text": ...}]"""
    import torch

    tok_ko, model_ko, device = _load_ko_summarizer()
    inputs = tok_ko(text, return_tensors="pt", max_length=1024, truncation=truncation)
    ...
    with torch.no_grad():
        out_ids = model_ko.generate(**inputs, **generation_args)

    summary = tok_ko.decode(
        out_ids[0],
...
def summarize_en(text, max_length=60, min_length=20):
    import torch

    tok_en, model_en, device = _load_en_summarizer()
    inputs = tok_en(text, return_tensors="pt", max_length=1024, truncation=True)
    inputs = {k: v.to(device) for k, v in inputs.items()}
...
companies_data = {
    "Company": ["Samsung", "SK Hynix", "LG Electronics", "Kakao", "Naver", "Krafton"],
    "Sector": ["Semiconductor", "Semiconductor", "Electronics", "Internet", "Internet", "Games"],
    "Revenue": ["302", "44", "83", "8", "10", "5"],
    "Employees": ["270000", "30000", "75000", "12000", "14000", "5000"],
    "Founded": ["1969", "1983", "1958", "2010", "1999", "2007"],
}

question1 = "Which company has the highest revenue?"
question2 = "What is the average number of employees?"
questions = [
    "Which company was founded first?",
    "How many companies are in the Internet sector?",
    "What is the total number of employees?",
]

sports_data = {
    "Team": ["Korea", "Japan", "Australia", "China", "UAE"],
    "Wins": ["7", "6", "5", "4", "2"],
    "Losses": ["1", "2", "3", "4", "6"],
    "Goals For": ["21", "18", "15", "12", "8"],
    "Points": ["21", "18", "15", "12", "6"],
}
...
final_question = "Which company has the most employees?"
